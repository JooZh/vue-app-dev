...
function router(path = "/api") {
  if (path.split("/").length > 2) {
    throw new Error(
      'custom routing formats support only one hierarchy. eg: "/api" '.error
    );
  ...
  // 验证版本
  exec("npm info music-api-for-qq version", (err, stdout) => {
    if (!err) {
      let onlineVersion = stdout.trim();
      let localVersion = packageJson.version;
      console.log(
        `[music-api] latest version: ${onlineVersion}, current version: ${localVersion}, please update timely.`
          .debug
      );
    }
  });
...
function server(options) {
  ...
  if (path.indexOf("/") !== 0) {
    throw new Error(
      'The route must be opened with "/". eg: "/music/api"'.error
    );
  }
  // 解析当前传入的路由
  ...
  if (cache && typeof cache === "boolean") {
    ...
    server.use(
      apicache(`${cache} minutes`, (req, res) => res.statusCode === 200)
    );
  } else {
  ...
  // 输出请求接口信息
  console.log(
    `[music-api] server start: http://${host}:${port + path}/... `.debug
  );
