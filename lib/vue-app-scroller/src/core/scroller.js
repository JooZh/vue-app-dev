...
class Scroller {
  ...
  _initSelector(selector) {
    ...
    if (observerMutationSupport) {
      let observer = new MutationObserver(mutations => {
        ...
        // if (mutations[0].target.className === "scroller--content") {
        //   m.scrollY = 0;
        //   m.scrollX = 0;
        // }
  ...
  _doTouchCanDeceleration() {
    ...
    for (
      ...
      i > 0 && m._touchArr[i].t > m._lastTouchT - 100;
      i -= 1
    ) {
      startIndex = i;
    }
    ...
    if (endIndex !== startIndex) {
      // 计算这两点之间的相对运动
      let timeOffset = m._touchArr[endIndex].t - m._touchArr[startIndex].t;
      let movedX = m.scrollX - m._touchArr[startIndex].x;
      let movedY = m.scrollY - m._touchArr[startIndex].y;
      // 基于50ms计算每个渲染步骤的移动
