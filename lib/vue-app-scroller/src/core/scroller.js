...
class Scroller {
  ...
  _initEvent() {
    ...
    // 判断是否支持触摸事件
    const supportTouch =
      (m.w.Modernizr && !!m.w.Modernizr.touch) ||
      (() => {
        return !!(
          "ontouchstart" in m.w ||
          (m.w.DocumentTouch && document instanceof window.DocumentTouch)
        );
      })();
    const _event = {
  ...
  _doTouchCanDeceleration() {
    ...
    for (
      let i = endPos;
      i > 0 && m._touchArr[i] > m._lastTouchT - 100;
      i -= 3
    ) {
      startPos = i;
    }
  ...
  emit(eventType, ...args) {
    let m = this;
    if (m._handles.hasOwnProperty(eventType)) {
      m._handles[eventType].forEach(item => {
  ...
  _publish(left, top, animate) {
    ...
    if (animate && m.ops.animating) {
      ...
      let animatType = wasAnimating
        ? utils._easeOutCubic
        : utils._easeInOutCubic;
  ...
  _startDeceleration() {
    ...
    let verify = () => {
      let shouldContinue =
        Math.abs(m._velocityX) >= minVelocity ||
        Math.abs(m._velocityY) >= minVelocity;
      if (!shouldContinue) {
